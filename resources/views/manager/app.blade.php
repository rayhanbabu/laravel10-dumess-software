...
@section('app'.$category,'active')
@section('content')


@if($category=='Application' || $category=='Session')
<div class="row mt-3 mb-0 mx-2">
  <div class="col-sm-6 my-2">
     @if($category=='Application') 
        <h4 class="mt-0">Application View </h4>
     @elseif($category=='Session')
        <h4 class="mt-0">Session View </h4>
      @else @endif
  </div>
    ...
    <table id="employee_data" class="table table-bordered table-hover">
      <thead>
        <tr>
          ...
          <th width="35%" class="sorting" data-sorting_type="asc" data-column_name="phone" style="cursor: pointer">
        ...
        @elseif($category=='Session')
             Session description
         @else @endif
             <span id="phone_icon"><i class="fas fa-sort-amount-up-alt"></i></span>
          </th>
...
<script>
  $(document).ready(function() {
    ...
    function fetch() {
      $.ajax({
        type: 'GET',
        url: '/manager/app_fetch/{{$category}}',
        datType: 'json',
    ...
    $(document).on('click', '.delete_id', function(e) {
      ...
      if (confirm("Are you sure you want to delete this?")) {
        $.ajax({
          type: 'DELETE',
          url: '/manager/app_delete/' + delete_id,
          success: function(response) {
    ...
    $(document).on('submit', '#edit_form', function(e) {
       ...
       $.ajax({
        type: 'POST',
        url: '/manager/app_update/' + edit_id,
        data: editData,
        contentType: false,
        processData: false,
        beforeSend: function() {
    ...
    function fetch_data(page, sort_type = "", sort_by = "", search = "") {
      $.ajax({
        url: "/manager/app/{{$category}}/fetch_data?page=" + page + "&sortby=" + sort_by + "&sorttype=" + sort_type + "&search=" + search,
        ...
        <div class="modal-body p-4 bg-light">
         ...
         @elseif($category=='Session')
             ...
             <label><b>Session/Building/Location</b></label><br><br>
                 <input name="phone" id="edit_phone"    type="text" class="form-control" required /><br>
      ...
      <form method="post" id="add_form" enctype="multipart/form-data" >
        ...
        <div class="modal-body p-4 bg-light">
          ...
          @elseif($category=='Session')
              ...
              <label><b>Session/Building/Location</b></label><br><br>
              <input name="phone" id="phone"   type="text" class="form-control" required /><br>
          @else @endif
